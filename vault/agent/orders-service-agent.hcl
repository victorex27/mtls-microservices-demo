...
auto_auth {
  method "approle" {
    ...
    config = {
      role_id_file_path                   = "/vault/agent/creds/orders-service/role_id"
      secret_id_file_path                 = "/vault/agent/creds/orders-service/secret_id"
  ...
  sink "file" {
    config = {
      path = "/tmp/orders-service-vault-token"
    }
  }
}

template {
  source      = "/vault/agent/templates/orders-service-bundle.json.tpl"
