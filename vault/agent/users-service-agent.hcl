...
auto_auth {
  method "approle" {
    ...
    config = {
      role_id_file_path                   = "/vault/agent/creds/users-service/role_id"
      secret_id_file_path                 = "/vault/agent/creds/users-service/secret_id"
  ...
  sink "file" {
    config = {
      path = "/tmp/users-service-vault-token"
    }
  }
}

template {
  source      = "/vault/agent/templates/users-service-bundle.json.tpl"
